...
public class ScriptField_Particle extends android.renderscript.Script.FieldBase {
    ...
    public static Element createElement(RenderScript rs) {
        ...
        eb.add(Element.F32(rs), "pointSize");
        return eb.create();
    }
    ...
    public synchronized void set_pointSize(int index, float v, boolean copyNow) {
        if (mIOBuffer == null) mIOBuffer = new FieldPacker(Item.sizeof * getType().getX()/* count */);
        if (mItemArray == null) mItemArray = new Item[getType().getX() /* count */];
        if (mItemArray[index] == null) mItemArray[index] = new Item();
        mItemArray[index].pointSize = v;
        if (copyNow)  {
    ...
    public synchronized float get_pointSize(int index) {
        if (mItemArray == null) return 0;
        return mItemArray[index].pointSize;
    }

...
import com.android.rs.test.structBitCode;

/**
 * @hide
 */
public class ScriptField_Point2 extends android.renderscript.Script.FieldBase {
    ...
    private  ScriptField_Point2(RenderScript rs) {
        mItemArray = null;
        mIOBuffer = null;
        mElement = createElement(rs);
    }

    public  ScriptField_Point2(RenderScript rs, int count) {
        mItemArray = null;
        mIOBuffer = null;
        mElement = createElement(rs);
        init(rs, count);
    ...
    public  ScriptField_Point2(RenderScript rs, int count, int usages) {
        mItemArray = null;
        mIOBuffer = null;
        mElement = createElement(rs);
    ...
    public static ScriptField_Point2 create1D(RenderScript rs, int dimX, int usages) {
        ScriptField_Point2 obj = new ScriptField_Point2(rs);
        obj.mAllocation = Allocation.createSized(rs, obj.mElement, dimX, usages);
        return obj;
    }

    public static ScriptField_Point2 create1D(RenderScript rs, int dimX) {
        return create1D(rs, dimX, Allocation.USAGE_SCRIPT);
    }

    public static ScriptField_Point2 create2D(RenderScript rs, int dimX, int dimY) {
        return create2D(rs, dimX, dimY, Allocation.USAGE_SCRIPT);
    }

    public static ScriptField_Point2 create2D(RenderScript rs, int dimX, int dimY, int usages) {
        ScriptField_Point2 obj = new ScriptField_Point2(rs);
    ...
    public static ScriptField_Point2 createCustom(RenderScript rs, Type.Builder tb, int usages) {
        ScriptField_Point2 obj = new ScriptField_Point2(rs);
        Type t = tb.create();
